//! Implements PBKDF2 using HMAC-SHA512.
//! This file was copied from [rust-bitcoin/rust-bip39](https://github.com/rust-bitcoin/rust-bip39/blob/master/src/pbkdf2.rs).
//! The contents in this file were written by Steven Roose <steven@stevenroose.org> and distributed
...
fn mnemonic_write_into<'a, M>(mnemonic: M, engine: &mut sha512::HashEngine)
where
    M: Iterator<Item = &'a str> + Clone,
{
    for (i, word) in mnemonic.enumerate() {
...
pub(self) fn create_hmac_engine<'a, M>(mnemonic: M) -> hmac::HmacEngine<sha512::Hash>
where
    M: Iterator<Item = &'a str> + Clone,
{
...
pub(crate) fn pbkdf2<'a, M>(mnemonic: M, unprefixed_salt: &[u8], c: usize, res: &mut [u8])
where
    M: Iterator<Item = &'a str> + Clone,
{
